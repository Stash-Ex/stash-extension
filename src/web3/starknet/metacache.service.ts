...
export const callCachesAtLocation = (
    ...
    console.log("callCachesAtLocation: ", json.stringify(res))
...
export const callGetCache = (contract: Contract) => async (
    ...
    const locationFelt = shortString.encodeShortString(location);
    const res = await contract.call("getCache", [locationFelt, cacheId])
    console.log("got getCache: " + JSON.stringify(res))
    const { cache } = res as any;
    console.log("Cache hint", json.stringify(cache.hint))
    return {
        location: location,
        id: cacheId,
        token: cache?.token ? number.toHex(cache.token) : "No token!",
        amount: uint256.uint256ToBN(cache?.amount).toString(),
        key: cache?.key ? number.toHex(cache.key) : "No key!",
        hint: cache?.hint ? shortString.decodeShortString(number.toHex(cache.hint)) : "No Hints!",
        owner: cache?.owner ? number.toHex(cache.owner) : "No owner!",
